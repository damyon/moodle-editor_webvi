...
// Call to boot strap after all functions are loaded
//webvi_bootstrap();
M.editor_webvi = M.editor_webvi || {

    init : function(params) {
        var textarea = Y.one('#' +params);
        var hiddenInput = webvi_createHiddenInputFromTextarea(textareaNode);
        var canvas = webvi_createCanvasFromTextarea(textareaNode);

        var parentEle = eles[i].parentNode;
        parentEle.replaceChild(canvas, textareaNode);
        parentEle.appendChild(hiddenInput);

        var state = new webvi_state(hiddenInput, canvas, textareaNode);
        webvi_stateList.push(state);
        webvi_renderCanvas(state);
        // register a keyboard listener for this canvas
        canvas.addEventListener('keydown', webvi_handleDocumentKeyboardInput, false);
